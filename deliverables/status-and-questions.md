...
| 3 | Hub upload spike — variant dirs vs our flat layout | Ben | Samir | p1 | 1–2 | Week 3 | **Done (packaging) / Blocked (delivery)** | Packaging solved: `scripts/build_hub_repo.py` emits the spec-compliant layout, loads on hardware, `neuron` is a documented Hub backend type. Uploaded `bbioren/neuron-rmsnorm`. Delivery blocked: kernels need `repo_type="kernel"` and creation is access-restricted (**403**, Finding #35). Downstream verified via `LOCAL_KERNELS`: 9/9 swapped, 9 NKI calls, 0 fallbacks, `cos_sim 1.000000` |
...
| 3 | **Who asks HF for kernel-repo creation access for an AWS org?** This is now the binding blocker and it sits *before* the trusted-publisher flag. Without it there is no kernel repo to publish to or point a mapping entry at | Pinak | Samir, John | **p0** | Hub delivery + the mapping PR | Ask for creation access first, `trustedKernelPublisher` second. `trust_remote_code=True` bypasses trust, **not** creation — so the earlier "we can just bypass it" answer does not apply here |
